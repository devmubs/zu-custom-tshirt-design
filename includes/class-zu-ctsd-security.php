...
class ZU_CTSD_Security {
    ...
    public static function sanitize_design_data(array $data): array {
        ...
        if (isset($data['elements']) && is_array($data['elements'])) {
            // Enforce a hard limit of 50 elements to prevent DoS
            $elements = array_slice($data['elements'], 0, 50, true);

            $sanitized['elements'] = [];
            foreach ($elements as $key => $element) {
                if (is_array($element)) {
                    $sanitized['elements'][$key] = self::sanitize_element($element);
                }
            }
        }
    ...
    public static function validate_file_upload(array $file): array {
        ...
        if (empty($mime_type)) {
            ...
            $errors[] = __('Could not verify file type securely.', 'zu-custom-tshirt');
        } elseif (!isset(self::$allowed_mime_types[$mime_type])) {
