...
class ZU_CTSD_REST_API {
    ...
    public function calculate_price(WP_REST_Request $request): WP_REST_Response {
        ...
        if (!ZU_CTSD_Security::check_rate_limit('calculate_price', 20, 60)) {
            return new WP_REST_Response([
                'success' => false,
        ...
        // Sanitize design data to prevent DoS and ensure valid format
        $raw_design_data = $params['design_data'] ?? [];
        $design_data = ZU_CTSD_Security::sanitize_design_data(is_array($raw_design_data) ? $raw_design_data : []);
