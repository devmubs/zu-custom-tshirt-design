...
## 2025-05-16 - Environment-Agnostic MIME Verification and DoS Mitigation
**Vulnerability:** Core file validation relied on `finfo_open` without checking for extension availability, risking fatal errors. Additionally, sanitization logic lacked element limits, exposing the server to DoS via oversized payloads.
**Learning:** Security helpers must be resilient to different PHP environments and enforce hard resource limits on user-controlled arrays to prevent memory exhaustion during processing.
**Prevention:** Always wrap `finfo` calls in `function_exists` checks, provide fallbacks like `mime_content_type`, and use `array_slice` to cap the size of processed arrays from external input.
